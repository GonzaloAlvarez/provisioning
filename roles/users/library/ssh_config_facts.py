...
DOCUMENTATION = '''
---
module: ssh_config_facts
short_description: Discover information on hosts from SSH config file
description:
  - Discover information on specific host within the SSH config file
options:
  user:
    description:
  ...
  host:
    description:
      - The endpoint this configuration we are looking up.
    required: true
    choices: []
'''
...
class ConfigParser(object):
    ...
    def load(self):
        ...
        self.config.parse(open(self.ssh_config_file))
        for entry in self.config.__dict__.get("_config"):
...
def main():
    ...
    host_identity_file=""
    results = config.config.lookup(host)
    if results and 'identityfile' in results and results['identityfile'] and len(results['identityfile']) > 0:
        host_identity_file=results['identityfile'][0]
